...
export default function InstagramConnectScreen() {
  ...
  const metaConfig = getMetaIntegrationConfig();
  ...
  async function handleOAuthConnect() {
    if (!user) {
      Alert.alert(
        'Önce giriş yap',
        'Instagram bağlantısı için önce Sipariş Kutusu hesabına giriş yapmalı veya kayıt olmalısın.',
        [{ text: 'Giriş / Kayıt Ol', onPress: () => router.push('/auth?redirect=/instagram-connect' as never) }]
      );
      return;
    }
    ...
    if (!hasStore) {
      Alert.alert(
        'Mağaza profili gerekli',
        'Instagram gönderilerini mağazana aktarmak için önce satıcı mağaza profilini oluşturmalısın.',
        [{ text: 'Mağaza Oluştur', onPress: () => router.push('/store-setup?connectInstagram=1' as never) }]
      );
      return;
    }
    ...
    if (!metaConfig.isConfigured) {
      Alert.alert(
        'Meta ayarı eksik',
        'Instagram bağlantısı için EXPO_PUBLIC_META_APP_ID ve EXPO_PUBLIC_META_REDIRECT_URI değerlerini .env dosyasına eklemelisin.'
      );
      return;
    }
  ...
  const connectCta = !user
    ? {
        icon: 'person-circle-outline',
        title: 'Önce Giriş / Kayıt Ol',
        subtitle: 'Instagram bağlantısı için hesap gerekli',
      }
    : !hasStore
    ? {
        icon: 'storefront-outline',
        title: 'Önce Mağaza Profilini Oluştur',
        subtitle: 'Gönderiler mağaza profiline aktarılır',
      }
    : {
        icon: 'logo-instagram',
        title: 'Instagram ile Oturum Aç',
        subtitle: 'İşletme veya içerik üretici hesabı',
      };
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: pal.bg }} edges={['top']}>
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14, backgroundColor: pal.headerBg, borderBottomWidth: 1, borderBottomColor: pal.headerBorder }}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={{ fontFamily: fonts.regular, fontSize: 12, color: pal.textSecondary }}>İşletme veya içerik üretici hesabını bağla</Text>
        </View>
          ...
          <>
            ...
            <View style={{ backgroundColor: pal.card, borderRadius: 20, padding: 20, borderWidth: 1, borderColor: pal.border }}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingTop: 12, borderTopWidth: 1, borderTopColor: pal.border }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                  ...
                  <View>
                    ...
                    <Text style={{ fontFamily: fonts.regular, fontSize: 11, color: pal.textSecondary }}>
                      Son senkronizasyon: {formatSyncTime(syncState.lastSyncAt)}
                    </Text>
                  </View>
                </View>
                <Pressable
                ...
                >
                  ...
                  <Text style={{ fontFamily: fonts.bold, fontSize: 12, color: colors.primary }}>Şimdi Senkronize Et</Text>
                </Pressable>
              </View>
            </View>
            ...
            <View style={{ backgroundColor: pal.warnBg, borderRadius: 14, padding: 14, borderWidth: 1, borderColor: pal.warnBorder }}>
              <Text style={{ fontFamily: fonts.medium, fontSize: 12, color: pal.warnText, textAlign: 'center', lineHeight: 18 }}>
                Erişim bilgilerin güvenli şekilde Supabase tarafında saklanır. İçeriklerine sadece sen erişebilirsin.
              </Text>
            </View>
          </>
        ) : (
          <>
            ...
            <View style={{ backgroundColor: pal.card, borderRadius: 20, padding: 28, alignItems: 'center', borderWidth: 1, borderColor: pal.border }}>
              ...
              <Text style={{ fontFamily: fonts.regular, fontSize: 14, color: pal.textSecondary, textAlign: 'center', marginTop: 10, lineHeight: 22 }}>
                İşletme veya içerik üretici hesabını bağlayarak gönderilerini otomatik ürüne dönüştür. İçeriklerini tek yerden yönet.
              </Text>
            </View>

            {/* Benefits */}
            {[
              { icon: 'images-outline', title: 'Gönderi & Reels İçe Aktar', desc: 'Mevcut içeriklerini mağazana aktar', color: '#6366F1' },
              { icon: 'sparkles-outline', title: 'Otomatik Ürün Taslağı', desc: 'Açıklamadan fiyat, kategori ve beden çıkar', color: '#F59E0B' },
            ...
            {!metaConfig.isConfigured ? (
              <View style={{ backgroundColor: pal.warnBg, borderRadius: 14, padding: 14, borderWidth: 1, borderColor: pal.warnBorder, flexDirection: 'row', gap: 10 }}>
                <Ionicons name="settings-outline" size={18} color={pal.warnText} />
                <Text style={{ fontFamily: fonts.medium, fontSize: 12, color: pal.warnText, flex: 1, lineHeight: 18 }}>
                  Meta App ID ve redirect URI eklenince gerçek OAuth akışını bağlayacağız. Şimdilik demo bağlantı korunuyor.
                </Text>
              </View>
            ) : null}
            <Pressable
              onPress={handleOAuthConnect}
              disabled={oauthLoading || !metaConfig.isConfigured}
              style={{ backgroundColor: '#E1306C', borderRadius: 16, padding: 18, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 12, opacity: oauthLoading || !metaConfig.isConfigured ? 0.55 : 1 }}
            >
              {oauthLoading ? (
                <>
                ...
                <>
                  <Ionicons name={connectCta.icon as any} size={24} color="#fff" />
                  <View>
                    <Text style={{ fontFamily: fonts.bold, fontSize: 16, color: '#fff' }}>{connectCta.title}</Text>
                    <Text style={{ fontFamily: fonts.regular, fontSize: 12, color: 'rgba(255,255,255,0.8)' }}>{connectCta.subtitle}</Text>
                  </View>
                </>
              )}
            </Pressable>
            ...
            <View style={{ backgroundColor: pal.warnBg, borderRadius: 14, padding: 14, flexDirection: 'row', gap: 10, borderWidth: 1, borderColor: pal.warnBorder }}>
              ...
              <Text style={{ fontFamily: fonts.regular, fontSize: 12, color: pal.warnText, flex: 1, lineHeight: 18 }}>
                Sadece işletme veya içerik üretici hesapları desteklenir. Kişisel hesaplar bağlanamaz. Erişim bilgilerin güvenli şekilde saklanır.
              </Text>
            </View>
          </>
        )}
