...
export default function CartScreen() {
  ...
  const productTitle = params.title ? decodeURIComponent(params.title) : 'Seçilen İlan';
  ...
  function startConversation() {
    if (!sellerId) {
      Alert.alert('Satıcı bilgisi eksik', 'Bu ilan için satıcı bilgisi bulunamadı. İlandan tekrar dene.');
      return;
    }
    ...
    const lines = [
      `İlan görüşmesi #${draftId}`,
      `İlan: ${productTitle}`,
      `Adet: ${quantity}`,
      normalizedPrice > 0 ? `İlandaki Fiyat: ${formatCurrency(normalizedPrice)}` : 'İlandaki Fiyat: Mesajda netleşecek',
      selectedAddress ? `Bulusma/Teslim referansi: ${selectedAddress.title} - ${selectedAddress.city}` : '',
      city ? `Sehir: ${city}` : '',
      district ? `Ilce: ${district}` : '',
      selectedContactNote ? `İletişim Notum: ${selectedContactNote.brand || 'Not yok'}` : '',
      note ? `Ek Not: ${note}` : '',
      'Platform ödeme ve kargo takibi yapmaz. Detayları burada netleştirebilir miyiz?',
    ].filter(Boolean);
  ...
  function openWhatsApp() {
    if (!whatsappContact) {
      Alert.alert('Bilgi', 'Bu ilan için WhatsApp bağlantısı yok. Mesaj ekranından devam edebilirsin.');
      return;
    }

    const normalized = whatsappContact.replace(/\D/g, '');
    const text = encodeURIComponent(`${productTitle} ilanı için görüşebilir miyiz?`);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F8FAFC' }} edges={['top']}>
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#E2E8F0', backgroundColor: '#fff' }}>
        ...
        <Text style={{ fontFamily: fonts.headingBold, fontSize: 16, color: colors.textPrimary }}>İlan İletişim Taslağı</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 140 }}>
        <View style={{ backgroundColor: '#fff', borderRadius: 18, borderWidth: 1, borderColor: '#E2E8F0', padding: 14 }}>
          ...
          <Text style={{ fontFamily: fonts.regular, fontSize: 12, color: colors.textSecondary, marginTop: 6 }}>
            Bu akış sadece alıcı ve satıcıyı görüştürür. Ödeme ve teslimat taraflar arasında yapılır.
          </Text>
          <Text style={{ fontFamily: fonts.headingBold, fontSize: 18, color: colors.primary, marginTop: 8 }}>
            {normalizedPrice > 0 ? formatCurrency(normalizedPrice) : 'Fiyat Mesajda Netleşecek'}
          </Text>
        </View>
        ...
        {step === 'details' ? (
          <View style={{ marginTop: 12, backgroundColor: '#fff', borderRadius: 18, borderWidth: 1, borderColor: '#E2E8F0', padding: 14 }}>
            ...
            {savedContactNotes.length > 0 ? (
              <View style={{ marginTop: 12 }}>
                <Text style={{ fontFamily: fonts.bold, fontSize: 13, color: colors.textPrimary, marginBottom: 8 }}>Kayıtlı İletişim Notu (Opsiyonel)</Text>
                <View style={{ gap: 8 }}>
            ...
            <TextInput
              ...
              placeholder="Buluşma saati, iletişim tercihi, ürün durumu soruları..."
              placeholderTextColor={colors.textMuted}
              multiline
          ...
          <View style={{ marginTop: 12, backgroundColor: '#fff', borderRadius: 18, borderWidth: 1, borderColor: '#E2E8F0', padding: 14 }}>
            ...
            <View style={{ gap: 6 }}>
              ...
              {normalizedPrice > 0 ? (
                <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: colors.textSecondary }}>İlandaki Fiyat: {formatCurrency(normalizedPrice)}</Text>
              ) : null}
              {selectedAddress ? (
                <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: colors.textSecondary }}>Seçilen Konum: {selectedAddress.title}</Text>
              ) : null}
              ...
              {selectedContactNote ? (
                <Text style={{ fontFamily: fonts.regular, fontSize: 13, color: colors.textSecondary }}>
                  İletişim Notu: {selectedContactNote.brand || 'Kayıtlı not'}
                </Text>
              ) : null}
              <Text style={{ fontFamily: fonts.medium, fontSize: 12, color: colors.textMuted, marginTop: 2 }}>
                Platform ödeme ve kargo takibi sunmaz. Bu adım sadece iletişim mesajı başlatır.
              </Text>
            </View>
          </View>
        )}
      ...
      <View style={{ position: 'absolute', left: 0, right: 0, bottom: 0, backgroundColor: '#fff', borderTopWidth: 1, borderTopColor: '#E2E8F0', paddingHorizontal: 16, paddingTop: 10, paddingBottom: 18 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
          ...
          <Text style={{ fontFamily: fonts.headingBold, fontSize: 18, color: colors.primary }}>{normalizedPrice > 0 ? formatCurrency(total) : 'Mesajda netleşecek'}</Text>
        </View>
        ...
        {step !== 'confirm' ? (
          ...
          <View style={{ gap: 8 }}>
            ...
            >
              <Text style={{ fontFamily: fonts.bold, fontSize: 14, color: '#fff' }}>Mesajlaşmayı Başlat</Text>
            </Pressable>
            {whatsappContact ? (
              <Pressable
