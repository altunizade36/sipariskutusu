...
export default function PaymentMethodsScreen() {
  ...
  const loadMethods = useCallback(async () => {
    ...
    try {
      ...
      setMethods([]);
      setError(err instanceof Error ? err.message : 'Ödeme yöntemleri yüklenemedi.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function handleSave() {
    ...
    if (!canSave) {
      Alert.alert('Eksik bilgi', 'Lütfen tercih basligini gir.');
      return;
    }
    ...
    try {
      ...
      Alert.alert('Hata', err instanceof Error ? err.message : 'Ödeme yöntemi eklenemedi.');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleSetDefault(id: string) {
    try {
      ...
      Alert.alert('Hata', err instanceof Error ? err.message : 'Varsayılan ödeme yöntemi güncellenemedi.');
    }
  }

  function handleDelete(id: string) {
    Alert.alert('Ödeme yöntemini sil', 'Bu kayıt kaldırılacak. Devam edilsin mi?', [
      { text: 'Vazgec', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            ...
            Alert.alert('Hata', err instanceof Error ? err.message : 'Kayıt silinemedi.');
          }
        },
      },
    ]);
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#F7F7F7]" edges={['top']}>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 12, gap: 10 }}>
        {loading ? (
          <Text style={{ fontFamily: fonts.medium, fontSize: 13, color: colors.textSecondary }}>Ödeme tercihleri yükleniyor...</Text>
        ) : methods.length === 0 ? (
          <View className="rounded-2xl border border-dashed border-[#CBD5E1] bg-white px-4 py-10 items-center">
            <Ionicons name="card-outline" size={28} color={colors.primary} />
            <Text style={{ fontFamily: fonts.bold, fontSize: 14, color: colors.textPrimary, marginTop: 8 }}>Kayıtlı ödeme tercihi yok</Text>
            <Text style={{ fontFamily: fonts.regular, fontSize: 12, color: colors.textSecondary, marginTop: 4, textAlign: 'center' }}>
              Platform ödeme almaz. Sadece alıcı-satıcı arasında konuşurken kullanmak için tercih kaydı ekleyebilirsin.
            </Text>
              ...
              <View className="flex-row items-center justify-between">
                ...
                {method.is_default ? (
                  <View className="px-2 py-0.5 rounded-full" style={{ backgroundColor: '#DBEAFE' }}>
                    <Text style={{ fontFamily: fonts.bold, fontSize: 10, color: colors.primary }}>Varsayılan</Text>
                  </View>
                ) : null}
              </View>
              ...
              <View className="flex-row mt-3" style={{ gap: 8 }}>
                {!method.is_default ? (
                  <Pressable onPress={() => handleSetDefault(method.id)} className="px-3 py-1.5 rounded-lg" style={{ backgroundColor: '#EFF6FF' }}>
                    <Text style={{ fontFamily: fonts.bold, fontSize: 11, color: colors.primary }}>Varsayılan Yap</Text>
                  </Pressable>
                ) : null}
        ...
        <View className="flex-1 justify-end" style={{ backgroundColor: '#00000066' }}>
          <View className="bg-white rounded-t-3xl p-4" style={{ maxHeight: '82%' }}>
            <View className="flex-row items-center justify-between mb-3">
              <Text style={{ fontFamily: fonts.headingBold, fontSize: 16, color: colors.textPrimary }}>Yeni Ödeme Tercihi</Text>
            ...
            <ScrollView keyboardShouldPersistTaps="handled">
              <View className="mb-3">
                <Text style={{ fontFamily: fonts.bold, fontSize: 12, color: colors.textSecondary, marginBottom: 6 }}>Yöntem Türü</Text>
              ...
              <View className="mb-3 rounded-xl border border-[#BFDBFE] bg-[#EFF6FF] px-3 py-2.5">
                <Text style={{ fontFamily: fonts.medium, fontSize: 11, color: colors.primary }}>
                  Güvenlik notu: Platform kart veya ödeme tahsilatı yapmaz. Buradaki kayıtlar sadece alıcı-satıcı görüşmesini hızlandırmak için kullanılır.
                </Text>
              </View>
              ...
              >
                <Text style={{ fontFamily: fonts.medium, fontSize: 12, color: form.is_default ? colors.primary : colors.textPrimary }}>
                  {form.is_default ? 'Bu yöntem varsayılan olacak' : 'Varsayılan ödeme yöntemi yap'}
                </Text>
              </Pressable>
