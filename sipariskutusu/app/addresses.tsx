...
export default function AddressesScreen() {
  ...
  async function handleSave() {
    if (!canSave || saving) {
      if (!canSave) {
        Alert.alert('Eksik alan', 'Lütfen başlık, ad soyad, telefon, adres ve şehir alanlarini doldur.');
      }
      return;
    }
  ...
  async function handleSetDefault(addressId: string) {
    try {
      ...
      Alert.alert('Hata', err instanceof Error ? err.message : 'Varsayılan adres güncellenemedi.');
    }
  }
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#F7F7F7]" edges={['top']}>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 12, gap: 10 }}>
        {loading ? (
          <Text style={{ fontFamily: fonts.medium, fontSize: 13, color: colors.textSecondary }}>Adresler yükleniyor...</Text>
        ) : addresses.length === 0 ? (
          <View className="rounded-2xl border border-dashed border-[#CBD5E1] bg-white px-4 py-10 items-center">
            <Ionicons name="location-outline" size={28} color={colors.primary} />
            <Text style={{ fontFamily: fonts.bold, fontSize: 14, color: colors.textPrimary, marginTop: 8 }}>Kayıtlı adres yok</Text>
            <Text style={{ fontFamily: fonts.regular, fontSize: 12, color: colors.textSecondary, marginTop: 4, textAlign: 'center' }}>
              Sipariş adimlarinda hizli secim icin simdi bir adres ekleyebilirsin.
            </Text>
            ...
            <ScrollView keyboardShouldPersistTaps="handled">
              ...
              >
                <Text style={{ fontFamily: fonts.medium, fontSize: 12, color: form.is_default ? colors.primary : colors.textPrimary }}>
                  {form.is_default ? 'Bu adres varsayilan olacak' : 'Varsayılan adres olarak isaretle'}
                </Text>
              </Pressable>
