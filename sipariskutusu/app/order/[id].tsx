...
export default function OrderDetailScreen() {
  ...
  const loadOrder = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Sipariş detayi yüklenemedi.');
      setOrder(null);
    } finally {
      setLoading(false);
    }
  ...
  async function handleStatusUpdate(nextStatus: OrderStatus) {
    ...
    try {
      ...
      await loadOrder();
      Alert.alert('Basarili', 'Sipariş durumu güncellendi.');
    } catch (err) {
      Alert.alert('Hata', err instanceof Error ? err.message : 'Sipariş güncellenemedi.');
    } finally {
      setUpdating(false);
    }
  }
    ...
    <SafeAreaView className="flex-1 bg-[#F7F7F7]" edges={['top']}>
      <View className="bg-white px-3 py-2 border-b border-[#33333315] flex-row items-center">
        ...
        <Pressable
          ...
          accessibilityLabel="Siparişi yenile"
        >
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 12, gap: 10 }}>
        ...
        {!loading && order ? (
          <>
            ...
            <View className="rounded-xl border border-[#33333315] bg-white p-3">
              <Text style={{ fontFamily: fonts.bold, fontSize: 13, color: colors.textPrimary, marginBottom: 8 }}>Sipariş Kalemleri</Text>
            ...
            {canManage ? (
              <View className="rounded-xl border border-[#BFDBFE] bg-[#EFF6FF] p-3">
                <Text style={{ fontFamily: fonts.bold, fontSize: 13, color: colors.primary }}>Satıcı Sipariş Yönetimi</Text>
                <Text style={{ fontFamily: fonts.regular, fontSize: 11, color: colors.textSecondary, marginTop: 4 }}>
                  Durum güncellemeleri alıcıya bildirim olarak gider.
                </Text>
                ...
                <View className="flex-row flex-wrap mt-3" style={{ gap: 8 }}>
                  {availableActions.map((action) => (
                    <Pressable
                      ...
                      accessibilityRole="button"
                      accessibilityLabel={`Durumu ${action.label} olarak güncelle`}
                    >
                  ...
                  {availableActions.length === 0 ? (
                    <Text style={{ fontFamily: fonts.medium, fontSize: 11, color: colors.textSecondary }}>
                      Bu sipariş icin ek durum aksiyonu yok.
                    </Text>
                  ) : null}
                </View>
              </View>
