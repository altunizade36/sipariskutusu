...
import { setContentStatus, type ParsedProductDraft } from '../src/services/instagramService';
...
export default function InstagramQuickPublishScreen() {
  ...
  const { hasStore, reloadProducts } = useListings();
  ...
  async function handlePublish() {
    ...
    try {
      ...
      if (params.postId) {
        await setContentStatus(params.postId, { convertedProductId: listing.id });
        await linkInstagramStorePostToListing(params.postId, listing.id).catch(() => undefined);
      }
        ...
        <ScrollView contentContainerStyle={{ padding: 16, gap: 14, paddingBottom: 40 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <View style={{ backgroundColor: isDarkMode ? '#1E3A8A22' : '#EFF6FF', borderRadius: 14, padding: 12, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            ...
            <Text style={{ fontFamily: fonts.medium, fontSize: 12, color: colors.primary, flex: 1 }}>
              Açıklamadan otomatik analiz edildi. "TAHMİN EDİLDİ" etiketli alanları kontrol et ve gerekirse düzenle.
            </Text>
          </View>
