...
import { getT, locale } from '../src/i18n';
...
function formatDateLabel(iso: string): string {
  const messagesT = getT().messages;
  const date = new Date(iso);
  const today = new Date();
  const yesterday = new Date();
  yesterday.setDate(today.getDate() - 1);
  if (date.toDateString() === today.toDateString()) return messagesT.today;
  if (date.toDateString() === yesterday.toDateString()) return messagesT.yesterday;
  return date.toLocaleDateString(locale === 'en' ? 'en-US' : 'tr-TR', { day: 'numeric', month: 'long' });
}
 
function formatPrice(value?: number) {
  if (typeof value !== 'number' || value <= 0) return getT().messages.askPrice;
...
function mapListingStatus(status?: string) {
  const messagesT = getT().messages;
  if (status === 'sold') return messagesT.sold;
  if (status === 'archived') return messagesT.archived;
  return messagesT.active;
}
  ...
  return (
    <View style={{ backgroundColor: isDelivered ? '#F0FDF4' : '#F0F9FF', borderWidth: 1, borderColor: isDelivered ? '#86EFAC' : '#BAE6FD', borderRadius: 14, padding: 12, maxWidth: '82%' }}>
      <Text style={{ fontFamily: fonts.bold, fontSize: 11, color: isDelivered ? '#166534' : '#0369A1', marginBottom: 5 }}>
        {isDelivered ? '🎉 Süreç Sonuçlandı' : '📦 Görüşme Durumu'}
      </Text>
      <Text style={{ fontFamily: fonts.bold, fontSize: 13, color: colors.textPrimary }}>
      ...
      {isDelivered ? (
        <Text style={{ fontFamily: fonts.medium, fontSize: 11, color: '#166534', marginTop: 6 }}>
          Taraflar süreci görüşerek tamamlamıştır.
        </Text>
      ) : null}
    </View>
  );
...
function getQuickReplies() {
  return getT().messages.quickReplies;
}
...
export default function MessagesScreen() {
  ...
  const messageMaxWidth = Math.min(Math.max(width - 92, 220), 460);
  const attachmentSize = Math.min(Math.max(width * 0.56, 180), 260);
  ...
  const activeProductCard = useMemo(() => {
    ...
    return { id: routeProductId, title: relatedProductTitle, image: undefined, priceLabel: undefined, price: undefined, status: messagesT.active };
  }, [activeListingFromParams, conversation?.listing, routeProductId, relatedProductTitle, messagesT.active]);
  ...
  const dmCandidates = useMemo(() => {
    ...
    if (isRemoteMode) {
      remoteConversations.forEach((item) => {
        ...
        const displayName = profile?.full_name?.trim() || messagesT.newMessageFallback;
      ...
      allProducts.forEach((product) => {
        ...
        map.set(sellerId, {
          id: sellerId,
          name: product.brand?.trim() || messagesT.newMessageFallback,
  ...
  }, [allProducts, fallbackConversations, isRemoteMode, remoteConversations, user?.id, messagesT.newMessageFallback]);
  ...
  const linkedConversationCount = useMemo(
    () => conversations.filter((item) => Boolean(item.listing?.id || item.listing?.title)).length,
    [conversations],
  );
  ...
  function openVideoMeeting() {
    ...
    const roomKey = `siparişkutusu-${conversation.id.replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 24) || 'room'}`;
    const meetingUrl = `https://meet.jit.si/${roomKey}`;

    Alert.alert(
      'Görüntülü görüşme',
      'Toplantı bağlantısı açılacak. Karşı tarafla paylaşarak görüşmeyi başlatabilirsin.',
      [
        {
          text: 'Linki Paylas',
          onPress: () => {
            Share.share({
              message: `Görüntülü görüşme bağlantısı: ${meetingUrl}`,
            }).catch(() => undefined);
          },
        },
        {
          ...
          onPress: () => {
            Linking.openURL(meetingUrl).catch(() => {
              Alert.alert('Hata', 'Toplantı bağlantısı açılamadı.');
            });
          },
        },
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: palette.screenBg }} edges={['top']}>
      ...
      <View className="px-4 pt-3 pb-3 border-b" style={{ backgroundColor: palette.headerBg, borderBottomColor: palette.border }}>
        <View className="flex-row items-center justify-between">
          ...
          {conversation ? (
            <View className="flex-1 flex-row items-center">
              ...
              <View className="ml-2.5 flex-1">
                ...
                {isRemoteMode ? (
                    ...
                    {counterpartyPresence?.isOnline ? messagesT.online : formatLastSeenLabel(counterpartyPresence?.lastSeenAt)}
                  </Text>
                ) : (
                  <Text style={{ fontFamily: fonts.regular, fontSize: 12, color: palette.textSecondary }}>{messagesT.demoChat}</Text>
                )}
              </View>
            </View>
          ) : (
            <View style={{ flex: 1, minWidth: 0, marginHorizontal: 10 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                <Text style={{ fontFamily: fonts.headingBold, fontSize: 21, color: palette.textPrimary }} numberOfLines={1}>{messagesT.title}</Text>
                {totalUnread > 0 ? (
              ...
              <Text style={{ fontFamily: fonts.regular, fontSize: 11, color: palette.textSecondary, marginTop: 1 }} numberOfLines={1}>
                {messagesT.marketplaceInbox}
              </Text>
            </View>
          )}
          <View style={{ flexDirection: 'row', gap: 6 }}>
            <Pressable
              ...
              accessibilityRole="button"
              accessibilityLabel={conversation ? 'Sohbet işlemlerini ac' : 'Mesaj yardim menusu'}
            >
        ...
        {!conversation ? (
          <View style={{ marginTop: 12, borderRadius: 16, padding: 12, backgroundColor: isDarkMode ? '#172033' : '#EFF6FF', borderWidth: 1, borderColor: isDarkMode ? '#273449' : '#DBEAFE' }}>
            <Text style={{ fontFamily: fonts.bold, fontSize: 13, color: palette.textPrimary }}>{messagesT.inboxSubtitle}</Text>
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 10 }}>
              <View style={{ borderRadius: 999, backgroundColor: isDarkMode ? '#1F2937' : '#fff', paddingHorizontal: 10, height: 28, flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                <Ionicons name="cube-outline" size={14} color={colors.primary} />
                <Text style={{ fontFamily: fonts.medium, fontSize: 11, color: palette.textSecondary }}>
                  {linkedConversationCount} {messagesT.connectedToProduct}
                </Text>
              </View>
              <View style={{ borderRadius: 999, backgroundColor: isDarkMode ? '#1F2937' : '#fff', paddingHorizontal: 10, height: 28, flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                <Ionicons name="chatbubbles-outline" size={14} color={colors.primary} />
                <Text style={{ fontFamily: fonts.medium, fontSize: 11, color: palette.textSecondary }}>
                  {conversations.length} {messagesT.buyerSellerThread}
                </Text>
              </View>
            </View>
          </View>
        ...
        {!conversation ? (
          <View style={{ marginTop: 12, flexDirection: 'row', alignItems: 'center', backgroundColor: palette.headerChipBg, borderRadius: 16, paddingHorizontal: 14, height: 46, borderWidth: 1.5, borderColor: palette.border, gap: 10 }}>
            ...
            <TextInput value={searchText} onChangeText={setSearchText} placeholder={messagesT.searchConversations} placeholderTextColor={palette.textMuted} style={{ fontFamily: fonts.regular, fontSize: 14, color: palette.textPrimary, flex: 1 }} accessibilityLabel={messagesT.searchPlaceholder} />
            {searchText.length > 0 ? (
      ...
      {!conversation ? (
        <View className="flex-1">
          <ScrollView
            className="flex-1"
            contentContainerStyle={{ paddingBottom: 96, paddingTop: 10 }}
          ...
          >
            {filteredConversations.length === 0 ? (
              <EmptyChatState
                title={conversations.length > 0 ? messagesT.noResults : messagesT.noMessages}
                description={conversations.length > 0 ? messagesT.noResultsSub : messagesT.noMessagesLong}
                actionLabel={messagesT.startShopping}
                onAction={() => router.push('/')}
              />
            ) : (
              filteredConversations.map((item) => {
                const lastMessage = item.messages[item.messages.length - 1]?.text ?? item.lastMessage ?? messagesT.newMessageFallback;
                const listingImage = item.listing?.image;
                const listingPrice = item.listing ? formatPrice(item.listing.price) : undefined;
                ...
                return (
                  <Pressable
                    ...
                    style={({ pressed }) => ({
                      backgroundColor: pressed ? (isDarkMode ? '#1E293B' : '#F0F4FF') : (isDarkMode ? '#111827' : '#fff'),
                      marginHorizontal: 12,
                      marginBottom: 10,
                      paddingVertical: 13,
                      ...
                      borderRadius: 16,
                      borderWidth: 1,
                      borderColor: hasUnread ? colors.primary + '55' : (isDarkMode ? '#253044' : '#E5EAF3'),
                      shadowColor: '#000',
                  ...
                  >
                    ...
                    <View style={{ flex: 1, minWidth: 0 }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                        ...
                        <Text style={{ fontFamily: fonts.regular, fontSize: 11, color: hasUnread ? colors.primary : palette.textMuted, minWidth: 42, textAlign: 'right' }}>{formatTime(item.lastMessageAt)}</Text>
                      </View>
                      ...
                      {listingImage || listingPrice ? (
                        <View style={{ marginTop: 9, backgroundColor: isDarkMode ? '#172033' : '#F8FAFF', borderRadius: 12, paddingHorizontal: 9, paddingVertical: 8, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: isDarkMode ? '#334155' : '#DBEAFE' }}>
                          {listingImage
                          ...
                          <View style={{ flex: 1, minWidth: 0, marginLeft: 8 }}>
                            <Text style={{ fontFamily: fonts.bold, fontSize: 10, color: colors.primary }} numberOfLines={1}>{messagesT.productConnection}</Text>
                            <Text style={{ fontFamily: fonts.medium, fontSize: 11, color: palette.textSecondary, marginTop: 1 }} numberOfLines={1}>{item.listing?.title || messagesT.relatedProduct}</Text>
                          </View>
                          {listingPrice ? <Text style={{ fontFamily: fonts.bold, fontSize: 12, color: colors.primary, marginLeft: 8, maxWidth: 88 }} numberOfLines={1}>{listingPrice}</Text> : null}
                        </View>
                      ) : null}
                    </View>
                  </Pressable>
          ...
          <NewMessageButton onPress={() => setShowNewDmModal(true)} label={messagesT.newMessage} />
        </View>
      ) : null}
          ...
          <FlatList
            ...
            ListHeaderComponent={
              <View style={{ marginBottom: 12 }}>
                {activeProductCard ? (
                  <View style={{ backgroundColor: isDarkMode ? '#111827' : '#fff', borderRadius: 18, padding: 12, borderWidth: 1, borderColor: isDarkMode ? '#2B3548' : '#D8E2F2', shadowColor: '#000', shadowOpacity: isDarkMode ? 0 : 0.06, shadowRadius: 10, shadowOffset: { width: 0, height: 3 }, elevation: 2 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                      {activeProductCard.image ? (
                        <Image source={{ uri: activeProductCard.image }} style={{ width: 62, height: 62, borderRadius: 14, backgroundColor: '#F0F4F8' }} />
                      ) : (
                        <View style={{ width: 62, height: 62, borderRadius: 14, backgroundColor: isDarkMode ? '#1F2937' : '#F0F4F8', alignItems: 'center', justifyContent: 'center' }}>
                          <Ionicons name="cube-outline" size={22} color={colors.primary} />
                        </View>
                      )}
                      <View style={{ flex: 1, minWidth: 0, marginLeft: 11 }}>
                        <Text style={{ fontFamily: fonts.bold, fontSize: 11, color: colors.primary, marginBottom: 3 }} numberOfLines={1}>{messagesT.productConnection}</Text>
                        <Text style={{ fontFamily: fonts.bold, fontSize: 14, color: palette.textPrimary }} numberOfLines={2}>{activeProductCard.title}</Text>
                        <View style={{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 7, marginTop: 7 }}>
                          {activeProductCard.priceLabel ? (
                            <Text style={{ fontFamily: fonts.headingBold, fontSize: 16, color: colors.primary }} numberOfLines={1}>{activeProductCard.priceLabel}</Text>
                          ) : null}
                          <View style={{ flexDirection: 'row', alignItems: 'center', borderRadius: 999, backgroundColor: activeProductCard.status === messagesT.active ? '#DCFCE7' : '#FEF3C7', paddingHorizontal: 8, height: 24 }}>
                            <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: activeProductCard.status === messagesT.active ? '#16A34A' : '#D97706', marginRight: 5 }} />
                            <Text style={{ fontFamily: fonts.bold, fontSize: 10, color: activeProductCard.status === messagesT.active ? '#166534' : '#92400E' }} numberOfLines={1}>
                              {activeProductCard.status === messagesT.active ? messagesT.active : messagesT.listingInactive}
                            </Text>
                          </View>
                        </View>
                      </View>
                    ...
                    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', borderRadius: 999, backgroundColor: isDarkMode ? '#172033' : '#F1F5F9', paddingHorizontal: 10, height: 30 }}>
                        <Ionicons name="person-circle-outline" size={15} color={colors.primary} />
                        <Text style={{ fontFamily: fonts.medium, fontSize: 11, color: palette.textSecondary, marginLeft: 5 }}>{messagesT.sellerConnection}</Text>
                      </View>
                      <View style={{ flexDirection: 'row', alignItems: 'center', borderRadius: 999, backgroundColor: isDarkMode ? '#172033' : '#F1F5F9', paddingHorizontal: 10, height: 30 }}>
                        <Ionicons name="pricetag-outline" size={14} color={colors.primary} />
                        <Text style={{ fontFamily: fonts.medium, fontSize: 11, color: palette.textSecondary, marginLeft: 5 }}>{messagesT.offerSignal}</Text>
                      </View>
                      <View style={{ flexDirection: 'row', alignItems: 'center', borderRadius: 999, backgroundColor: isDarkMode ? '#172033' : '#F1F5F9', paddingHorizontal: 10, height: 30 }}>
                        <Ionicons name="receipt-outline" size={14} color={colors.primary} />
                        <Text style={{ fontFamily: fonts.medium, fontSize: 11, color: palette.textSecondary, marginLeft: 5 }}>{messagesT.orderSignal}</Text>
                      </View>
                    </View>

                    <View style={{ flexDirection: 'row', gap: 8, marginTop: 12 }}>
                      {activeProductCard.id ? (
                        <Pressable onPress={() => router.push(`/product/${activeProductCard.id}`)} style={{ flex: 1, backgroundColor: isDarkMode ? '#172033' : '#EFF6FF', borderRadius: 12, height: 40, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 6, borderWidth: 1, borderColor: isDarkMode ? '#2B3548' : '#DBEAFE' }}>
                          <Ionicons name="open-outline" size={15} color={colors.primary} />
                          <Text style={{ fontFamily: fonts.bold, fontSize: 12, color: colors.primary }} numberOfLines={1}>{messagesT.viewProduct}</Text>
                        </Pressable>
                      ) : null}
                      {activeProductCard.id ? (
                        <Pressable onPress={() => router.push(`/order/${activeProductCard.id}`)} style={{ flex: 1, backgroundColor: '#0F766E', borderRadius: 12, height: 40, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 6 }}>
                          <Ionicons name="receipt-outline" size={15} color="#fff" />
                          <Text style={{ fontFamily: fonts.bold, fontSize: 12, color: '#fff' }} numberOfLines={1}>{messagesT.startOrder}</Text>
                        </Pressable>
                      ) : null}
                      <Pressable onPress={() => setShowOfferModal(true)} style={{ flex: 1, backgroundColor: colors.primary, borderRadius: 12, height: 40, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 6 }}>
                        <Ionicons name="pricetag-outline" size={15} color="#fff" />
                        <Text style={{ fontFamily: fonts.bold, fontSize: 12, color: '#fff' }} numberOfLines={1}>{messagesT.makeOffer}</Text>
                      </Pressable>
                    </View>
                  </View>
                ) : (
                  <View style={{ backgroundColor: isDarkMode ? '#111827' : '#fff', borderRadius: 16, padding: 12, borderWidth: 1, borderColor: isDarkMode ? '#2B3548' : '#E5EAF3', flexDirection: 'row', alignItems: 'center' }}>
                    <Ionicons name="link-outline" size={18} color={colors.textMuted} />
                    <Text style={{ fontFamily: fonts.medium, fontSize: 12, color: palette.textSecondary, marginLeft: 8, flex: 1 }}>{messagesT.noProductConnected}</Text>
                  </View>
                )}
                <View style={{ backgroundColor: '#FFFBEB', borderRadius: 14, paddingHorizontal: 12, paddingVertical: 8, marginTop: 8, flexDirection: 'row', alignItems: 'center' }}>
                  <Ionicons name="shield-checkmark-outline" size={14} color="#92400E" style={{ marginRight: 6 }} />
                  <Text style={{ fontFamily: fonts.medium, fontSize: 11, color: '#92400E', flex: 1 }}>{messagesT.safetyNotice}</Text>
                </View>
              </View>
            }
            ListFooterComponent={
              <TypingIndicator visible={(!isRemoteMode && typingConversationId === conversation.id) || (isRemoteMode && Boolean(remoteTypingUserId))} label={messagesT.typing} />
            }
            renderItem={({ item }) => {
              ...
              return (
                <View className={`mb-3 ${mine ? 'items-end' : 'items-start'}`}>
                  {item.msgKind === 'offer' && item.offerData ? (
                    ...
                    <Pressable onLongPress={() => openMessageActions(item)} delayLongPress={320} style={{ maxWidth: messageMaxWidth }}>
                      <Image source={{ uri: item.imageUri }} style={{ width: attachmentSize, height: attachmentSize, borderRadius: 14, opacity: item.status === 'sending' ? 0.6 : 1 }} />
                    </Pressable>
                  ) : (
                    <Pressable onLongPress={() => openMessageActions(item)} delayLongPress={320} style={{ maxWidth: messageMaxWidth }}>
                      <View style={{ flexDirection: 'row', alignItems: 'flex-end', maxWidth: messageMaxWidth }}>
                        ...
                        <View style={{ backgroundColor: mine ? '#1E5FC6' : (isDarkMode ? '#2D3748' : '#ECECEC'), opacity: item.status === 'sending' ? 0.7 : 1, maxWidth: mine ? messageMaxWidth : messageMaxWidth - 27, minWidth: 44, borderRadius: 20, borderTopLeftRadius: mine ? 20 : 5, borderTopRightRadius: mine ? 5 : 20, paddingHorizontal: 14, paddingVertical: 10 }}>
                        {item.replyToText ? (
                        ...
                        <Text style={{ fontFamily: fonts.medium, fontSize: 13, lineHeight: 18, color: mine ? '#fff' : colors.textPrimary }}>{visibleText}</Text>
                        </View>
                      </View>
                    </Pressable>
                  )}
                  ...
                  <View className="flex-row items-center mt-1 gap-1">
                    ...
                    {isEdited ? <Text style={{ fontFamily: fonts.regular, fontSize: 10, color: colors.textMuted }}>{messagesT.edited}</Text> : null}
                  ...
                  {mine && item.status === 'failed' ? (
                    <Pressable onPress={retryPendingMessage} className="mt-1.5">
                      <Text style={{ fontFamily: fonts.bold, fontSize: 10, color: '#DC2626' }}>{messagesT.sendFailedRetry}</Text>
                    </Pressable>
                  ) : null}
                </View>
              );
          ...
          {showAttachMenu ? (
            <View style={{ position: 'absolute', bottom: 80, left: 16, backgroundColor: '#fff', borderRadius: 14, padding: 8, borderWidth: 0.5, borderColor: '#E5E7EB', gap: 4, elevation: 4 }}>
              <Pressable onPress={handlePickAndSendImage} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, padding: 10, borderRadius: 10 }}>
                ...
                <Text style={{ fontFamily: fonts.medium, fontSize: 13, color: colors.textPrimary }}>{messagesT.attachGallery}</Text>
              </Pressable>
              <Pressable onPress={handlePickAndSendCameraImage} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, padding: 10, borderRadius: 10 }}>
                ...
                <Text style={{ fontFamily: fonts.medium, fontSize: 13, color: colors.textPrimary }}>{messagesT.attachCamera}</Text>
              </Pressable>
              <Pressable onPress={() => { setShowAttachMenu(false); setShowOfferModal(true); }} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, padding: 10, borderRadius: 10 }}>
                ...
                <Text style={{ fontFamily: fonts.medium, fontSize: 13, color: colors.textPrimary }}>{messagesT.priceOffer}</Text>
              </Pressable>
            </View>
          ) : null}
          ...
          <View style={{ backgroundColor: palette.composerBg, paddingHorizontal: 12, paddingTop: 10, paddingBottom: 14, borderTopWidth: 0.5, borderTopColor: palette.border }}>
            {replyTarget ? (
              <View className="mb-2 rounded-xl bg-[#F7F7F7] border border-[#33333315] px-3 py-2 flex-row items-center justify-between">
                <View className="flex-1 pr-2">
                  <Text style={{ fontFamily: fonts.bold, fontSize: 10, color: colors.primary }}>{messagesT.replying}</Text>
            ...
            {editTarget ? (
              <View className="mb-2 rounded-xl bg-[#FEF3C7] border border-[#FDE68A] px-3 py-2 flex-row items-center justify-between">
                <View className="flex-1 pr-2">
                  <Text style={{ fontFamily: fonts.bold, fontSize: 10, color: '#92400E' }}>{messagesT.editingMessage}</Text>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingVertical: 6, gap: 8 }} style={{ marginBottom: 8 }}>
                {getQuickReplies().map((qr) => (
                  <Pressable key={qr} onPress={() => handleQuickReply(qr)} style={{ backgroundColor: '#F0F4FF', borderRadius: 20, paddingHorizontal: 14, paddingVertical: 7, borderWidth: 0.5, borderColor: '#C7D7F9', marginRight: 6 }}>
                    <Text style={{ fontFamily: fonts.medium, fontSize: 12, color: colors.primary }} numberOfLines={1}>{qr}</Text>
                  </Pressable>
                ))}
              </ScrollView>
            ) : null}
            ...
            <View style={{ flexDirection: 'row', alignItems: 'flex-end', gap: 8 }}>
              ...
              <TextInput
                ...
                placeholder={messagesT.messageComposerHint}
                placeholderTextColor={palette.textMuted}
                multiline editable showSoftInputOnFocus autoCorrect autoCapitalize="sentences" blurOnSubmit={false}
                style={{ fontFamily: fonts.regular, fontSize: 14, color: palette.textPrimary, maxHeight: 104, minHeight: 44, flex: 1, minWidth: 0, backgroundColor: palette.inputBg, borderRadius: 22, paddingHorizontal: 16, paddingVertical: 10, borderWidth: 0.5, borderColor: palette.border, textAlignVertical: 'top' }}
                accessibilityLabel={messagesT.messageComposerHint}
              />
              {currentDraft ? (
                <Pressable
                  onPress={handleSend}
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.35)' }} pointerEvents="box-none">
          <KeyboardAvoidingView style={{ flex: 1, justifyContent: 'flex-end' }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
              <Text style={{ fontFamily: fonts.headingBold, fontSize: 18, color: colors.textPrimary }}>{messagesT.newMessage}</Text>
            ...
            <View className="mt-3 flex-row items-center bg-[#F7F7F7] rounded-xl px-3 h-11 border border-[#33333315]">
              ...
              <TextInput
                ...
                placeholder={messagesT.searchPeople}
                placeholderTextColor={colors.textMuted}
            ...
            {!newDmQuery.trim() && recentDmCandidates.length > 0 ? (
              <View style={{ marginTop: 10 }}>
                <Text style={{ fontFamily: fonts.bold, fontSize: 12, color: colors.textSecondary, marginBottom: 8 }}>{messagesT.quickStart}</Text>
            ...
            <ScrollView className="mt-3" contentContainerStyle={{ paddingBottom: 10 }} keyboardShouldPersistTaps="handled">
              {filteredDmCandidates.length === 0 ? (
                <View style={{ alignItems: 'center', paddingVertical: 28 }}>
                  ...
                  <Text style={{ fontFamily: fonts.bold, fontSize: 14, color: colors.textPrimary, marginTop: 10 }}>{messagesT.chatNotFound}</Text>
                  <Text style={{ fontFamily: fonts.regular, fontSize: 12, color: colors.textSecondary, marginTop: 4, textAlign: 'center' }}>
                    {messagesT.chatNotFoundSub}
                  </Text>
                </View>
              ) : (
                <>
                  {!newDmQuery.trim() && recentDmCandidates.length > 0 ? (
                    <>
                      <Text style={{ fontFamily: fonts.bold, fontSize: 12, color: colors.textSecondary, marginBottom: 4 }}>{messagesT.recentChats}</Text>
                        ...
                        >
                          ...
                          <View style={{ marginLeft: 10, flex: 1 }}>
                            ...
                            <Text style={{ fontFamily: fonts.regular, fontSize: 12, color: colors.textSecondary }} numberOfLines={1}>
                              {candidate.lastMessage ? truncateText(candidate.lastMessage, 38) : (candidatePresence[candidate.id] ? messagesT.online : messagesT.lastContact)}
                            </Text>
                          </View>
                  ...
                  {!newDmQuery.trim() ? <Text style={{ fontFamily: fonts.bold, fontSize: 12, color: colors.textSecondary, marginTop: 8, marginBottom: 4 }}>{messagesT.suggestedPeople}</Text> : null}
                  {suggestedDmCandidates.map((candidate) => (
                    ...
                    >
                      ...
                      <View style={{ marginLeft: 10, flex: 1 }}>
                        <Text style={{ fontFamily: fonts.bold, fontSize: 14, color: colors.textPrimary }} numberOfLines={1}>{candidate.name}</Text>
                        <Text style={{ fontFamily: fonts.regular, fontSize: 12, color: colors.textSecondary }} numberOfLines={1}>
                          {candidate.lastMessage ? truncateText(candidate.lastMessage, 38) : (candidatePresence[candidate.id] ? messagesT.online : messagesT.startMessaging)}
                        </Text>
                      </View>
                      <View style={{ alignItems: 'flex-end', gap: 4 }}>
                        {candidate.lastMessageAt ? <Text style={{ fontFamily: fonts.regular, fontSize: 11, color: colors.textMuted }}>{formatTime(candidate.lastMessageAt)}</Text> : null}
