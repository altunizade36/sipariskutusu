...
export default function ProductDetailScreen() {
  ...
  const localSellerKey = product.sellerId?.trim() || product.storeId?.trim() || product.brand?.trim() || '';
  ...
  useEffect(() => {
    ...
    const handleToggleFavorite = useCallback(async () => {
      if (!user && isSupabaseConfigured && canUseLiveListing) { router.push('/auth'); return; }
      const next = await toggleFav(product.id);
      setFavorited(next);
      setFavoriteCount((current) => Math.max(next ? current + 1 : current - 1, 0));
      refreshListing();
    }, [canUseLiveListing, refreshListing, router, user, product.id, toggleFav]);
  ...
  async function handleSendMessage() {
    ...
    if (!localSellerKey) {
      Alert.alert('Satıcı bilgisi eksik', 'Bu ürün için satıcı veya mağaza bağlantısı bulunamadı.');
      return;
    }

    if (product.sellerId && user.id === product.sellerId) {
  ...
  function handleStartOrderDraft() {
    if (!localSellerKey) {
      Alert.alert('Satıcı bilgisi eksik', 'Görüşme başlatmak için satıcı bilgisi bulunamadı.');
      return;
    }

    const query = `/cart?sellerId=${encodeURIComponent(localSellerKey)}&productId=${encodeURIComponent(product.id)}&title=${encodeURIComponent(product.title)}&price=${encodeURIComponent(String(product.price || 0))}&brand=${encodeURIComponent(product.brand || '')}&whatsapp=${encodeURIComponent(contactWhatsapp || '')}`;
