...
import { linkInstagramStorePostToListing } from '../src/services/storePostService';
...
export default function InstagramContentScreen() {
  ...
  const { hasStore, importInstagramContentToStore, reloadProducts } = useListings();
  ...
  const loadAll = useCallback(async () => {
    const [content, sm] = await Promise.all([
      syncInstagramContent().catch(async () => ({
        posts: await fetchInstagramPosts(),
        reels: await fetchInstagramReels(),
        stories: await fetchInstagramStories(),
      })),
      getContentStatusMap(),
    ]);
    const { posts: p, reels: r, stories: s } = content;
    setPosts(p);
    setReels(r);
    setStories(s);
    setStatusMap(sm);
  ...
  }, [hasStore, importInstagramContentToStore]);
  ...
  async function handleBulkPublish() {
    ...
    for (const p of readyPosts) {
      try {
        ...
        const listing = await submitListingToSupabase({
          title: d.title,
          ...
          coverIndex: 0,
          imageUris: [p.mediaUrl],
          negotiable: false,
          stock: 1,
          sourceType: 'instagram_import',
        });
        await setContentStatus(p.id, { convertedProductId: listing.id });
        await linkInstagramStorePostToListing(p.id, listing.id).catch(() => undefined);
        published++;
      } catch {}
    }
    if (published > 0) {
    ...
    return (
      ...
      >
        ...
        <View style={{ padding: 9 }}>
          ...
          <Text style={{ fontFamily: fonts.regular, fontSize: 9, color: pal.textMuted, marginBottom: 7 }}>
    ...
    return (
      <View style={{ backgroundColor: pal.card, borderRadius: 18, overflow: 'hidden', borderWidth: 1, borderColor: pal.border, marginBottom: 12 }}>
        ...
        <View style={{ padding: 14 }}>
          ...
          <Text style={{ fontFamily: fonts.regular, fontSize: 11, color: pal.textMuted, marginBottom: 12 }}>
