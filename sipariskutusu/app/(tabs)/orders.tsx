...
export default function OrdersScreen() {
  ...
  function openOrderConversation(order: Order) {
    if (activeTab === 'buying' && order.seller_id) {
      const firstItemTitle = order.order_items?.[0]?.title ?? 'Sipariş';
      ...
      router.push(buildSellerMessagesRoute({
        ...
        initialMessage: `#${order.id.slice(0, 8)} siparişim hakkinda güncelleme alabilir miyim?`,
      }));
      return;
    }
  ...
  return (
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.screenBg }} edges={['top']}>
      <View className="px-4 pt-2 pb-3 border-b" style={{ backgroundColor: palette.surfaceBg, borderBottomColor: palette.cardBorder }}>
        <Text style={{ fontFamily: fonts.headingBold, fontSize: 20, color: palette.textPrimary }}>Görüşme Geçmişi</Text>
        <Text style={{ fontFamily: fonts.regular, fontSize: 12, color: palette.textSecondary, marginTop: 3 }}>
          Satin aldigin ve sattigin siparişleri buradan yonetebilirsin.
        </Text>
          ...
          {TAB_OPTIONS.map((tab) => {
            ...
            return (
              <Pressable
                ...
                accessibilityLabel={`${tab.label} sipariş sekmesini ac`}
              >
        ...
        <TextInput
          ...
          placeholder="İlan no, ürün veya kişi ara"
          ...
          accessibilityLabel="Sipariş arama alani"
        />
      ...
      >
        {loading ? (
          <Text style={{ fontFamily: fonts.medium, fontSize: 13, color: palette.textSecondary }}>Siparişler yükleniyor...</Text>
          ...
          <View className="rounded-2xl border border-dashed px-4 py-10 items-center" style={{ borderColor: palette.border, backgroundColor: palette.surfaceBg }}>
            ...
            <Text style={{ fontFamily: fonts.bold, fontSize: 14, color: palette.textPrimary, marginTop: 8 }}>
              {activeTab === 'buying' ? 'Henuz satin alim yok' : 'Henuz satis siparişi yok'}
            </Text>
            <Text style={{ fontFamily: fonts.regular, fontSize: 12, color: palette.textSecondary, marginTop: 4, textAlign: 'center' }}>
              {activeTab === 'buying'
                ? 'Verdigin siparişler burada gorunecek.'
                : 'Mağazana gelen siparişler burada listelenecek.'}
            </Text>
          </View>
        ) : (
            ...
            return (
              <View key={order.id} className="rounded-xl border p-3" style={{ borderColor: palette.cardBorder, backgroundColor: palette.surfaceBg }}>
                ...
                <Text style={{ fontFamily: fonts.medium, fontSize: 12, color: palette.textPrimary, marginTop: 6 }}>
                  {activeTab === 'buying' ? 'Satıcı ile görüşme' : 'Alıcı ile görüşme'}
                </Text>
                ...
                <View className="flex-row mt-3" style={{ gap: 8 }}>
                  <Pressable
                    ...
                    accessibilityRole="button"
                    accessibilityLabel="Sipariş detayina git"
                  >
                    <Text style={{ fontFamily: fonts.bold, fontSize: 11, color: palette.textSecondary }}>
                      Detay
                    </Text>
                  ...
                  <Pressable
                    ...
                    accessibilityRole="button"
                    accessibilityLabel={activeTab === 'buying' ? 'Satıcı ile mesajlaşmayı aç' : 'Alıcı ile mesajlaşmayı aç'}
                  >
                    <Text style={{ fontFamily: fonts.bold, fontSize: 11, color: colors.primary }}>
                      {activeTab === 'buying' ? 'Satıcı ile Konuş' : 'Alıcı ile Konuş'}
                    </Text>
                  </Pressable>
                  ...
                  {activeTab === 'selling' && getSellerNextStatus(order.status) ? (
                    <Pressable
                      ...
                      accessibilityRole="button"
                      accessibilityLabel="Sipariş durumunu bir sonraki aşamaya güncelle"
                    >
                      <Text style={{ fontFamily: fonts.bold, fontSize: 11, color: '#fff' }}>
                        {quickUpdatingOrderId === order.id ? 'Güncelleniyor...' : 'Durum İlerle'}
                      </Text>
                    </Pressable>
                  ) : null}
                </View>
