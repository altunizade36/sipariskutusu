...
import { parseCaption } from '../../src/services/instagramService';
import { fetchStorePosts } from '../../src/services/storePostService';
...
function isUnlinkedInstagramPost(post: { source?: string; linkedProductId?: string }) {
  return post.source === 'instagram' && !post.linkedProductId;
}
...
export default function StoreScreen() {
  ...
  const [remoteStorePosts, setRemoteStorePosts] = useState<StorePost[]>([]);
  ...
  useEffect(() => {
    if (!viewingOtherStore || !liveStoreData?.id) {
      setRemoteStorePosts([]);
      return;
    }

    let cancelled = false;
    fetchStorePosts(liveStoreData.id)
      .then((posts) => {
        if (!cancelled) setRemoteStorePosts(posts);
      })
      .catch(() => {
        if (!cancelled) setRemoteStorePosts([]);
      });

    return () => { cancelled = true; };
  }, [liveStoreData?.id, viewingOtherStore]);
  ...
  const handleDeleteProduct = useCallback((productId: string, productTitle: string) => {
    Alert.alert(
      'İlanı Sil',
      `"${productTitle}" ilanını mağazandan kaldırmak istiyor musun?`,
      [
        { text: 'Vazgeç', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: () => {
            removeListing(productId).catch(() => {
              Alert.alert('Silinemedi', 'İlan silinirken bir sorun oluştu. Lütfen tekrar dene.');
            });
          },
        },
      ],
  ...
  const otherStorePosts = useMemo<StorePost[]>(
    () =>
  ...
  const currentStore = viewingOtherStore
    ? {
        ...
        username: resolvedOtherStore?.username || `@${(name || 'mağaza').replace(/\s+/g, '').toLowerCase()}`,
  ...
  const handleShareStore = useCallback(async () => {
    try {
      ...
      const shareUrl = handle ? `https://siparişkutusu.app/store/${handle}` : 'https://siparişkutusu.app';
      await Share.share({
  ...
  const activeStorePosts = viewingOtherStore ? (remoteStorePosts.length > 0 ? remoteStorePosts : otherStorePosts) : storePosts;
  ...
  const activeReelHasLocalEngagement = Boolean(activeReel?.linkedProductId);
  ...
  function openInstagramPostAsProduct(post: typeof activeStorePosts[number]) {
    const caption = post.caption || post.title;
    router.push({
      pathname: '/instagram-quick-publish',
      params: {
        postId: post.sourceId || post.id,
        mediaUrl: post.image,
        caption,
        draft: JSON.stringify(parseCaption(caption)),
      },
    } as never);
  }
        ...
        {!user ? (
          <View style={{ paddingHorizontal: 16, paddingBottom: 10 }}>
            <Pressable
              ...
              accessibilityLabel="Giriş yap veya kayıt ol"
            >
        ...
        <View style={{ paddingHorizontal: 16, paddingTop: 12 }}>
          <View style={{ backgroundColor: palette.surfaceBg, borderRadius: 24, padding: 20, borderWidth: 1, borderColor: palette.border }}>
            ...
            <Pressable
              ...
              accessibilityRole="button"
              accessibilityLabel={user ? 'Mağaza aç' : 'Giriş yapıp mağaza aç'}
            >
      ...
      >
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 12 }}>
          ...
          {user ? (
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
              ...
              <Pressable onPress={handleShareStore} accessibilityRole="button" accessibilityLabel="Mağazayi paylas">
            ...
            <Pressable
              ...
              accessibilityRole="button"
              accessibilityLabel="Giriş veya kayıt"
            >
        ...
        <View style={{ paddingHorizontal: 16, paddingBottom: 16 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 14 }}>
            ...
            <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-around' }}>
              <Pressable onPress={() => setActiveTab('products')} style={{ alignItems: 'center' }} accessibilityRole="button" accessibilityLabel="Ürünler sekmesini aç">
              ...
              <Pressable
                onPress={() => {
                  if (!isOwnStoreView) {
                    Alert.alert('Bilgi', 'Takipci listesi sadece mağaza sahibine acik.');
                    return;
                  }
              ...
              <Pressable
                onPress={() => {
                  if (!isOwnStoreView) {
                    Alert.alert('Bilgi', 'Takip edilenler listesi sadece mağaza sahibine acik.');
                    return;
                  }
          ...
          <View style={{ flexDirection: 'row', gap: 8, marginBottom: 4 }}>
            ...
            accessibilityRole="button"
            accessibilityLabel={viewingOtherStore ? (isFollowingCurrentStore ? 'Mağazayi takipten cik' : 'Mağazayi takip et') : 'Mağazayi duzenle'}>
            ...
            accessibilityRole="button"
            accessibilityLabel={viewingOtherStore ? 'Mağazaya mesaj gonder' : 'Mesaj kutusunu ac'}>
        ...
        <View style={{ flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: palette.border }}>
          {([
            ...
            { key: 'stories', icon: 'images-outline', label: 'Paylaşımlar' },
                ...
                {activeReels.map((post) => (
                  <Pressable key={post.id} style={{ width: GRID_ITEM, height: GRID_ITEM * 1.28 }} onPress={() => openReel(post.id)}>
                    ...
                    <View style={{ position: 'absolute', top: 8, right: 8, backgroundColor: '#00000077', borderRadius: 999, padding: 4 }}>
                      <Ionicons name={post.isVideo ? 'play' : post.source === 'instagram' ? 'logo-instagram' : 'images-outline'} size={13} color="#fff" />
                    </View>
                    {isOwnStoreView && isUnlinkedInstagramPost(post) ? (
                      <Pressable
                        onPress={(event) => {
                          event.stopPropagation();
                          openInstagramPostAsProduct(post);
                        }}
                        style={{
                          position: 'absolute',
                          left: 6,
                      ...
                      >
                        ...
                        <Text style={{ fontSize: 10, color: '#fff', fontFamily: fonts.bold }} numberOfLines={1}>
                          Ürüne Çevir
                        </Text>
                      </Pressable>
                    ) : null}
              ...
              >
                ...
                <View style={{ flexDirection: 'row', gap: 8 }}>
                  {[
                    ...
                    { label: 'Otomatik Taslak', icon: 'flash-outline', desc: 'Açıklama analizi' },
            ...
            <View style={{ flex: 1 }}>
              ...
              <View style={{ flex: 1, backgroundColor: '#00000055', justifyContent: 'space-between', padding: 16 }}>
                ...
                <View>
                  ...
                  ) : isOwnStoreView && isUnlinkedInstagramPost(activeReel) ? (
                    <Pressable
                      onPress={() => {
                        openInstagramPostAsProduct(activeReel);
                        setActiveReelId(null);
                      }}
                      style={{ marginTop: 10, backgroundColor: colors.primary, borderRadius: 12, paddingHorizontal: 12, paddingVertical: 11, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6 }}
                    >
