...
export default function StoreSetupScreen() {
  ...
  const params = useLocalSearchParams<{ connectInstagram?: string }>();
  const { user } = useAuth();
  ...
  async function handleCreateStore() {
    ...
    try {
      ...
      router.replace(params.connectInstagram === '1' ? '/instagram-connect' : '/(tabs)/store');
    } catch (error) {
  ...
  function renderContactStage() {
    return (
      <>
        ...
        {[
          { label: 'E-posta *', value: email, setValue: setEmail, placeholder: 'Örn. mağaza@email.com', keyboardType: 'email-address' as const },
          ...
          { label: 'Website', value: website, setValue: setWebsite, placeholder: 'Örn. https://mağazaniz.com', keyboardType: 'url' as const },
