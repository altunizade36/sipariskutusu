...
type LegalAcceptancePayload = {
  ...
  kvkkDisclosure: boolean;
  platformLiability: boolean;
  instagramImportNotice?: boolean;
  documents: string[];
};
...
const DEMO_ACCOUNTS = {
  buyer: {
    email: 'alici@siparişkutusu.demo',
    password: 'Demo123!',
    fullName: 'Demo Alıcı',
  },
  seller: {
    email: 'satici@siparişkutusu.demo',
    password: 'Demo123!',
...
const THEME_STORAGE_KEY = '@siparişkutusu/theme_dark_mode';
...
function assertBackendAuthSessionPayload(payload: unknown): BackendAuthSessionPayload {
  if (!payload || typeof payload !== 'object') {
    throw new Error('Auth cevabı geçersiz formatta.');
  }
  ...
  if (parsed.accessToken !== undefined && typeof parsed.accessToken !== 'string') {
    throw new Error('Auth cevabında accessToken string olmalı.');
  }

  if (parsed.refreshToken !== undefined && typeof parsed.refreshToken !== 'string') {
    throw new Error('Auth cevabında refreshToken string olmalı.');
  }
  ...
  if (
    ...
    throw new Error('Auth cevabında requiresEmailVerification boolean olmalı.');
  }

  if (parsed.message !== undefined && typeof parsed.message !== 'string') {
    throw new Error('Auth cevabında message string olmalı.');
  }
...
function assertBackendAuthOkPayload(payload: unknown) {
  if (!payload || typeof payload !== 'object') {
    throw new Error('Auth işlem cevabı geçersiz formatta.');
  }

  const parsed = payload as { ok?: unknown; message?: unknown };

  if (parsed.ok !== undefined && typeof parsed.ok !== 'boolean') {
    throw new Error('Auth işlem cevabında ok boolean olmalı.');
  }

  if (parsed.message !== undefined && typeof parsed.message !== 'string') {
    throw new Error('Auth işlem cevabında message string olmalı.');
  }
  ...
  const value = useMemo<AuthContextValue>(() => {
    ...
    return {
      ...
      async signInWithPassword(email, password) {
        ...
        if (!isSupabaseConfigured) {
          throw new Error(
            'Demo giriş için: alici@siparişkutusu.demo veya satici@siparişkutusu.demo, şifre: Demo123!',
          );
        }
        ...
        async signUpWithPassword(email, password, fullName, accountRole = 'buyer', legalAcceptance) {
        const normalizedEmail = email.trim().toLowerCase();
      ...
      async toggleDarkMode() {
        ...
        await AsyncStorage.setItem(THEME_STORAGE_KEY, next ? '1' : '0').catch(() => {
          // Kayıt hatası ana akışı bloklamamalı.
        });
      },
