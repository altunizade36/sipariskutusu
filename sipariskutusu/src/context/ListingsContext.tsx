...
import { fetchStorePosts, importInstagramStorePosts } from '../services/storePostService';
...
type ListingsContextValue = {
  ...
  importInstagramContentToStore: (items: InstagramStorePostInput[]) => void;
...
function uniqueStorePosts(items: StorePost[]) {
  const seen = new Set<string>();
  return items.filter((item) => {
    const key = item.sourceId ? `${item.source}:${item.sourceId}` : item.id;
    if (seen.has(key)) {
      return false;
    }
    seen.add(key);
...
export function ListingsProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!isSupabaseConfigured || !hasStore || !isUuid(activeStore.id)) {
      return;
    }

    let active = true;
    fetchStorePosts(activeStore.id)
      .then((remotePosts) => {
        if (!active || remotePosts.length === 0) return;
        setStorePosts((current) => uniqueStorePosts([...remotePosts, ...current]));
      })
      .catch((error) => {
        captureError(error, { scope: 'store_posts_load' });
      });

    return () => { active = false; };
  }, [activeStore.id, hasStore]);
  ...
  const storeProducts = useMemo(() => {
    ...
    const ownBackendProducts = backendProducts.filter((product) =>
      product.sellerId === user?.id ||
      product.storeId === activeStore.id ||
      product.brand === activeStore.name
    );

    return uniqueProductsById([...publishedListings, ...storyLinkedProducts, ...ownBackendProducts]);
  }, [activeStore.id, activeStore.name, backendProducts, hasStore, publishedListings, storyLinkedProducts, user?.id]);
  ...
  function addListing(listing: CreateListingInput) {
    ...
    const createdProduct: Product = {
      ...
      sellerId: listing.sellerId ?? user?.id ?? activeStore.id,
      storeId: listing.storeId ?? activeStore.id,
      title: listing.title,
  ...
  async function removeListing(id: string) {
    ...
    setBackendProducts((current) => current.filter((product) => product.id !== id));
  ...
  const importInstagramContentToStore = useCallback((items: InstagramStorePostInput[]) => {
    if (!hasStore || items.length === 0) {
      return;
    }

    const nextPosts: StorePost[] = items.map((item) => {
      const titleWords = item.caption
        .split(/\s+/)
      ...
      return {
        ...
        title: titleWords || 'Instagram paylaşımı',
        date: item.timestamp,
        type: 'product',
        likes: 0,
        comments: 0,
        isVideo: Boolean(item.isVideo || item.mediaType === 'VIDEO'),
        source: 'instagram',
        sourceId: item.id,
    ...
    setStorePosts((current) => {
      const seen = new Set(current.map((post) => post.sourceId ? `${post.source}:${post.sourceId}` : post.id));
      const fresh = nextPosts.filter((post) => !seen.has(`${post.source}:${post.sourceId}`));
      return fresh.length > 0 ? [...fresh, ...current] : current;
    });

    if (isSupabaseConfigured && user && !user.id.startsWith('demo-')) {
      importInstagramStorePosts(items)
        .then((savedPosts) => {
          if (savedPosts.length === 0) return;
          setStorePosts((current) => uniqueStorePosts([...savedPosts, ...current]));
        })
        .catch((error) => {
          captureError(error, { scope: 'instagram_store_posts_import' });
        });
    }
  }, [hasStore, user]);
