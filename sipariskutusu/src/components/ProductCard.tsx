...
const BADGE_MAP: Record<string, BadgeStyle> = {
  Flash:             { bg: '#FEF3C7', text: '#92400E', border: '#FDE68A', icon: '!', label: 'Anlık İndirim' },
  ...
  'En Çok İncelenen':{ bg: '#F0FDF4', text: '#166534', border: '#BBF7D0', icon: 'o', label: 'Çok İncelenen' },
...
function ProductCardComponent({ product, width = '100%' }: Props) {
  ...
  useEffect(() => {
    ...
    return () => { alive = false; };
  }, [checkFavorited, product.id, user?.id]);
