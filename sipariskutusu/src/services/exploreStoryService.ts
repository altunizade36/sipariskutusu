  ...
  return (data ?? [])
    .map((row: any) => {
      ...
      return {
        ...
        sellerName: storyRow.profiles?.full_name?.trim() || 'Satıcı',
        sellerAvatar:
...
export async function fetchSellerPeriodLeaderboard(
  ...
  return (data ?? []).map((row: any) => {
    ...
    return {
      sellerId: row.seller_id,
      sellerName: row.profiles?.full_name?.trim() || 'Satıcı',
      sellerAvatar: row.profiles?.avatar_url || '',
      storeName: row.stores?.name || row.profiles?.full_name || 'Mağaza',
