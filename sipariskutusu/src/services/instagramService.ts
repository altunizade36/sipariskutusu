...
import { getMetaIntegrationConfig } from './metaConfig';
...
type InstagramOAuthExchangeResponse = {
  ok: boolean;
  message?: string;
  account?: {
...
export function parseCaption(caption: string): ParsedProductDraft {
  ...
  const priceMatch =
    caption.match(/₺\s?(\d+(?:[.,]\d{3})*(?:[.,]\d{1,2})?)/i) ||
    caption.match(/\b(\d+(?:[.,]\d{3})*(?:[.,]\d{1,2})?)\s?(?:tl|₺|lira)\b/i) ||
    caption.match(/fiyat[ı:]\s*(\d+(?:[.,]\d{3})*(?:[.,]\d{1,2})?)/i);
...
function encodeOAuthState(payload: Record<string, string>) {
  try {
    const encoder = (globalThis as typeof globalThis & { btoa?: (value: string) => string }).btoa;
    if (!encoder) return '';
...
export async function connectInstagramOAuth(): Promise<InstagramConnection> {
  ...
  if (!config.isConfigured) {
    throw new Error('Meta App ID veya redirect URI eksik.');
  }
  if (!isSupabaseConfigured) {
    throw new Error('Instagram OAuth için Supabase yapılandırması gerekli.');
  }

  const oauthClientId = config.authProvider === 'instagram' ? config.instagramAppId : config.appId;
  const authUrl = new URL(
    config.authProvider === 'instagram'
      ? 'https://www.instagram.com/oauth/authorize'
      : `https://www.facebook.com/${config.graphApiVersion}/dialog/oauth`
  );
  authUrl.searchParams.set('client_id', oauthClientId as string);
  authUrl.searchParams.set('redirect_uri', config.redirectUri as string);
  authUrl.searchParams.set('response_type', 'code');
  authUrl.searchParams.set('scope', config.scopes.join(','));
  const state = encodeOAuthState({ returnTo: config.appReturnUri as string });
  if (state) {
    authUrl.searchParams.set('state', state);
  }
  if (config.authProvider === 'instagram') {
    authUrl.searchParams.set('enable_fb_login', '0');
    authUrl.searchParams.set('force_authentication', '1');
  }

  const result = await WebBrowser.openAuthSessionAsync(authUrl.toString(), config.appReturnUri as string);
  if (result.type !== 'success' || !result.url) {
    throw new Error('Instagram bağlantısı iptal edildi.');
  }

  const returnedUrl = new URL(result.url);
  const code = returnedUrl.searchParams.get('code');
  const error = returnedUrl.searchParams.get('error_description') || returnedUrl.searchParams.get('error');
  if (error) {
    throw new Error(error);
  }
  if (!code) {
    throw new Error('Meta OAuth kodu alınamadı.');
  }

  return completeInstagramOAuth(code, config.redirectUri, config.authProvider);
}
...
export async function completeInstagramOAuth(
  ...
  const config = getMetaIntegrationConfig();
  const effectiveRedirectUri = redirectUri?.trim() || config.redirectUri;
  const effectiveProvider = provider ?? config.authProvider;

  if (!effectiveRedirectUri) {
    throw new Error('Instagram redirect URI eksik.');
  }
  if (!isSupabaseConfigured) {
    throw new Error('Instagram OAuth için Supabase yapılandırması gerekli.');
  }
  ...
  const { data, error: fnError } = await supabase.functions.invoke<InstagramOAuthExchangeResponse>(
  ...
  if (!data?.ok || !data.account) {
    throw new Error(data?.message ?? 'Instagram hesabı bağlanamadı.');
  }

  const account = data.account;
  const username = account.username ?? account.instagram_handle ?? '';
  const conn: InstagramConnection = {
    connected: true,
    accountId: account.instagram_user_id ?? `ig_${Date.now()}`,
    username,
  ...
  await setSyncState({ status: 'success', lastSyncAt: conn.connectedAt, error: null });
...
export async function syncInstagramContent(): Promise<{
  ...
  try {
    if (isSupabaseConfigured) {
      const supabase = getSupabaseClient();
      const { data, error } = await supabase.functions.invoke<InstagramSyncResponse>('instagram-sync', {
        body: {},
      });

      if (!error && data?.ok && data.media) {
        const posts = data.media.map((item): InstagramPost => ({
          id: item.id,
          ...
          status: 'auto_draft',
          parsedDraft: parseCaption(item.caption ?? ''),
        })).filter((item) => Boolean(item.mediaUrl));
        const reels = posts
          .filter((item) => item.mediaType === 'VIDEO')
          .map((item): InstagramReel => ({
            id: item.id,
      ...
      if (error || data?.message) {
        await setSyncState({ status: 'error', lastSyncAt: null, error: error?.message ?? data?.message ?? 'Senkronizasyon başarısız.' });
      }
    }
