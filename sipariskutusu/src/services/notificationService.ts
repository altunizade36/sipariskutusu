...
function assertDispatchInput(input: NotificationDispatchInput) {
  ...
  if (channels.includes('push')) {
    if (!input.push?.title?.trim() || !input.push?.body?.trim()) {
      throw new Error('Push bildirimi için başlık ve içerik zorunludur.');
    }
  }
  ...
  if (channels.includes('sms')) {
    ...
    if (recipients.length === 0 || !input.sms?.message?.trim()) {
      throw new Error('SMS gönderimi için alıcı listesi ve mesaj zorunludur.');
    }
  }
  ...
  if (channels.includes('email')) {
    ...
    if (recipients.length === 0 || !input.email?.subject?.trim() || !hasContent) {
      throw new Error('E-posta gönderimi için alıcı, konu ve içerik/şablon zorunludur.');
    }
  }
}
...
function validateDispatchResponse(payload: unknown): DispatchResponse {
  if (!payload || typeof payload !== 'object') {
    throw new Error('Bildirim dispatch cevabı geçersiz formatta.');
  }

  const parsed = payload as Partial<DispatchResponse>;
  if (typeof parsed.ok !== 'boolean') {
    throw new Error('Bildirim dispatch cevabında ok boolean olmalı.');
  }

  if (parsed.requestId !== undefined && typeof parsed.requestId !== 'string') {
    throw new Error('Bildirim dispatch cevabında requestId string olmalı.');
  }
