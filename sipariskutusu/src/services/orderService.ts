...
function assertOrderPayload(payload: unknown): Order {
  if (!payload || typeof payload !== 'object') {
    throw new Error('Sipariş cevabı geçersiz formatta.');
  }

  const parsed = payload as Partial<Order>;
  if (!parsed.id || typeof parsed.id !== 'string') {
    throw new Error('Sipariş cevabında id zorunlu.');
  }

  if (!parsed.status || typeof parsed.status !== 'string') {
    throw new Error('Sipariş cevabında status zorunlu.');
  }

  if (typeof parsed.total !== 'number' || !Number.isFinite(parsed.total)) {
    throw new Error('Sipariş cevabında total geçersiz.');
  }

  if (!parsed.created_at || typeof parsed.created_at !== 'string') {
    throw new Error('Sipariş cevabında created_at zorunlu.');
  }
...
function assertOrderArrayPayload(payload: unknown): Order[] {
  if (!Array.isArray(payload)) {
    throw new Error('Sipariş listesi dizi olmalıdir.');
  }
...
export async function createOrder(input: CreateOrderInput): Promise<Order> {
  if (!input.items.length) {
    throw new Error('Sipariş olusturmak icin en az bir urun secilmelidir.');
  }
  ...
  if (hasInvalidItem) {
    throw new Error('Sipariş kalemleri geçersiz. Miktar ve fiyat pozitif olmalıdir.');
  }
  ...
  if (itemsError) {
    ...
    throw new Error('Sipariş kalemleri kaydedilemedi. Sipariş geri alindi.');
  }
